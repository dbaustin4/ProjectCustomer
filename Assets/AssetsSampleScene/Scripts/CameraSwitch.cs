...
public class CameraSwitch : MonoBehaviour {
  ...
  private void SetCameraToCurrentTarget() {
    if (currentTarget >= 0 && currentTarget < targetTransforms.Count) { //Check if currentTarget is a valid number in targetTransforms list
  ...
  private void SetNextTarget() { //go to previous pos/rot from list
    if (isTransitioning)
      return; //exit current method from executing
  ...
  private void SetPreviousTarget() { //go to next pos/rot from list
    if (isTransitioning)
      return; //exit current method from executing

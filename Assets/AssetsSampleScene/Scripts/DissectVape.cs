...
public class DissectVape : MonoBehaviour
{
    ...
    IEnumerator EnableDissectionAfterVoiceLine(float delay)
    { //creates a delay before you can dissect next obj
        yield return new WaitForSeconds(delay);
        canDissect = true;
    }

    IEnumerator DisappearAfterDelay(GameObject obj, float delay)
    { //make obj disappear after a delay
        yield return new WaitForSeconds(delay);
